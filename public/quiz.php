...
// 1. Recuperiamo il nome corretto dalla sessione (assicurati che sia 'pseudo' o 'username')
$userName = $_SESSION['user_name'] ?? 'Invitato';

// 2. Recuperiamo il punteggio totale dell'utente dal database
$userId = $_SESSION['user_id'];
$stmtScore = $pdo->prepare("SELECT COALESCE(SUM(points), 0) as total FROM score WHERE user_id = ?");
$stmtScore->execute([$userId]);
$userScore = $stmtScore->fetchColumn();
...
<main class="relative z-10 max-w-3xl mx-auto px-4 py-10 text-center">
    ...
    <div class="max-w-4xl mx-auto px-4 mt-10">
        ...
        <section class="w-full bg-white border-[3px] border-[#FF9F43] rounded-[30px] p-6 md:p-10 flex flex-col md:flex-row justify-between items-center shadow-[0_10px_30px_rgba(0,0,0,0.1)] mb-10">
            <div class="text-center md:text-left mb-6 md:mb-0">
                <h2 class="text-3xl md:text-4xl font-bold text-[#2D3436]">Bienvenue !</h2>
                <p class="text-2xl md:text-3xl font-bold text-[#33658A] mt-1">
                    <?= htmlspecialchars($userName) ?> 👋
                </p>
            </div>

            <div class="bg-white border-4 border-[#B8D1E1] px-8 py-4 rounded-2xl flex items-center gap-4 shadow-[5px_5px_0px_0px_#B8D1E1]">
                <span class="text-3xl">⭐</span>
                <span class="text-3xl font-bold text-[#2D3436]">
                    <?= number_format((int)$userScore, 0, '.', ' ') ?> pts
                </span>
            </div>
        </section>
            ...
            <?php else: ?>
                <?php foreach ($quizzes as $quiz): ?>
                    <div class="bg-white border-[3px] border-[#FF9F43] rounded-[30px] p-5 md:p-6 flex items-center gap-5 md:gap-8 shadow-md hover:shadow-xl transition-all group">
                        ...
                        <a href="../process/preparation_quiz.php?id=<?= $quiz['id'] ?>"
                            class="block text-center w-full sm:inline-block sm:w-auto  bg-[#FF9F43] hover:bg-[#FF8C1A] text-white font-bold py-2 px-6 md:py-3 md:px-14 rounded-2xl border-b-[6px] border-[#CC7A29] active:border-b-0 active:translate-y-1.5 transition-all text-lg md:text-xl uppercase tracking-widest">
                            Play
                        </a>
                    </div>
                <?php endforeach; ?>
    ...
    <div class="absolute -bottom-64 left-4 md:right-20 lg:right-40 z-10 hidden sm:block">
